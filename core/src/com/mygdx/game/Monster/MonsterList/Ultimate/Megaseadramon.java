...
public class Megaseadramon extends Pet{
    ...
    public Megaseadramon(int attack, int hp, int max_hp, int age, int hunger, int max_hunger, int stamina, int max_stamina, NormalAttack normalAttack, SpecialAttack specialAttack, Sleep sleep, Pup pup, int battleWins) {
        super(attack, hp, max_hp, age, hunger, max_hunger, stamina, max_stamina, normalAttack, specialAttack, sleep, pup, battleWins);
    }

    public Megaseadramon()
    {
        this.attack = 200;
        this.hp = 700;
        this.age = 36;
    ...
    public Megaseadramon(Pet petBefore)
    {
        super(petBefore);
    }
    ...
    @Override
    public Pet evolve(Pet Megaseadramon) {
        if (Megaseadramon.getAttack() >= 590 && Megaseadramon.getMax_hp() >= 1600 && Megaseadramon.getAge() >= 41 && Megaseadramon.getBattleWins() >= 50){
